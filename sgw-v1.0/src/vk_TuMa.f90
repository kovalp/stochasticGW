...
!                                                   
! Tuckerman Martyna
subroutine prep_vk( nx,ny,nz,dx,dy,dz,vk,periodic)
  ...
  !
  ! For periodic, ignore all of it and just set 4pi/k^2 as kernel

  cvk = 0d0
  periodc: if(.not.periodic) then
  ...
  kzdo : do iz=1,nz
     kydo : do iy=1,ny
        kxdo : do ix=1,nx
           ...
           periodicb : if(.not.periodic) then
 ...
 if(rank==0)then
    if(periodic) write(17,*)' aa ( tuma ),r0 = ',aa,r0
    write(17,*)' minval, maxval vk ',minval(vk), maxval(vk)
    call flush(17)
 endif
